...
# 6 Configure early stopping patience
    ...
    # datagen.fit(X_train)
    train_data_generator = datagen.flow(X_train, y_train, batch_size=config.batch_size,save_to_dir='./image_gen/',save_format='jpg')
    valid_data_generator = datagen.flow(X_valid, y_valid, batch_size=8,save_to_dir='./image_gen/',save_format='jpg')
    ...
    mobilenet_history = model.fit(train_data_generator, steps_per_epoch=len(X_train) / config.batch_size, callbacks=[checkpointer, earlystopper,WandbCallback()],
                                             epochs=config.epochs, verbose=1, validation_data=valid_data_generator)
# 7 Configure epochs
